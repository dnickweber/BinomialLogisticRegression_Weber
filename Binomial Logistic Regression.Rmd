...
#Just ensuring that R realizes these variables are categorical and not continuous...
blacktip_raw$Mortality <- factor(blacktip_raw$Mortality)
blacktip_raw$Condition <- factor(blacktip_raw$Condition)
blacktip_raw$Sex <- factor(blacktip_raw$Sex)
blacktip_raw$Hook.Location <- factor(blacktip_raw$Hook.Location)
```

```{r}
#Cleaning dataset: Removing 2 rows (1 shark without i-STAT blood data and 1 shark with no known mortality outcome because tags were removed before release)...
blacktip_cleaned <- blacktip_raw[-c(5,6), ]
blacktip_cleaned$Mortality <- factor(blacktip_cleaned$Mortality)

#Selecting which variables I want to include in PCA...
...
#This PCA is telling me to choose either 'pH' or 'Lactate', and either 'Handling Time' or 'Fork Length' for regression purposes. This makes sense because 'pH' and 'Lactate' are negatively correlated (as they should be) and 'Handling Time' and 'Fork Length' (and really 'Fight Time') are positively correlated, which makes sense (i.e. larger sharks are both on the line and handled for longer period of time). So based on the results of this PCA, I can get rid of 2 of 6 variables for the regression, and I am choosing to get rid of 'Lactate' and 'Fork Length'...
```

```{r}
#Predicting mortality for all sharks combined (i.e. combining sharks caught from shore and sharks caught from charter boats), with 3 mortalities and 29 survivors.
