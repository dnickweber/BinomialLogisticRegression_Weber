...
autoplot(prcomp(scale(pca_dataframe_naomitted)), shape=FALSE, label.size=4, loadings=TRUE, loadings.label=TRUE, loadings.label.size=4, loadings.colour='black', loadings.label.colour='black', data=blacktip_cleaned, colour='Mortality') + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
plot(blacktip_cleaned$pH, blacktip_cleaned$Mortality.Num, xlab="pH", ylab="Probability of Mortality", pch=19, xlim=rev(range(blacktip_cleaned$pH)), cex=1.2)
